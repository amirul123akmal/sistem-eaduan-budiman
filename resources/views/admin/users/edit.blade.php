...
@section('content')
	<h1 class="mb-6 text-2xl font-semibold">Edit Admin</h1>

	<form action="{{ route('admin.admins.update', $admin) }}" method="POST" class="max-w-2xl space-y-4">
		@csrf
		@method('PUT')

		<div>
			<label class="mb-1 block text-sm font-medium">Nama</label>
			<input name="name" value="{{ old('name', $admin->name) }}" class="w-full rounded border px-3 py-2 text-sm" />
		...
		<div class="grid gap-4 sm:grid-cols-2">
			<div>
				...
				<input name="phone_number" value="{{ old('phone_number', $admin->phone_number) }}" class="w-full rounded border px-3 py-2 text-sm" />
				@error('phone_number')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
			</div>
			<div>
				<label class="mb-1 block text-sm font-medium">Jawatan</label>
				<input name="position" value="{{ old('position', $admin->position) }}" class="w-full rounded border px-3 py-2 text-sm" />
		...
		<div>
			...
			<input type="email" name="email" value="{{ old('email', $admin->email) }}" class="w-full rounded border px-3 py-2 text-sm" />
			@error('email')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
		</div>

		<div class="grid gap-4 sm:grid-cols-2">
			<div>
				<label class="mb-1 block text-sm font-medium">Kata Laluan (biar kosong jika tidak tukar)</label>
