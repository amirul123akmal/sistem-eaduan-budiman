...
@section('content')
	<div class="mb-8">
		<div class="flex items-center justify-between">
			<div>
				<h1 class="text-3xl font-bold text-gray-900 mb-2">Kemaskini Jenis Aduan</h1>
				<p class="text-sm text-gray-600">ID: #{{ $complaintType->id }}</p>
			</div>
		...
		<div class="bg-gradient-to-r from-[#F0F7F0] to-[#F0F7F0]/80 border-b border-gray-200 px-6 py-4">
			<div class="flex items-center gap-3">
				...
				<div>
					...
					<p class="text-xs text-gray-600">Ubah maklumat jenis aduan di bawah</p>
				</div>
			</div>
		</div>
		<form action="{{ route(isset($isAdminPanel) && $isAdminPanel ? 'admin.panel.complaint-types.update' : 'admin.complaint-types.update', $complaintType) }}" method="POST" class="p-6" id="complaintTypeForm" novalidate>
			@csrf
			@method('PUT')
			...
			<div class="space-y-6">
				...
				<div>
					...
					<input 
						...
						name="type_name" 
						id="type_name" 
						value="{{ old('type_name', $complaintType->type_name) }}" 
						required
						maxlength="100"
						pattern="[a-zA-Z0-9\s.,-]+"
				...
				<div>
					...
					>{{ old('description', $complaintType->description) }}</textarea>
					<div class="mt-2 flex items-center justify-between">
