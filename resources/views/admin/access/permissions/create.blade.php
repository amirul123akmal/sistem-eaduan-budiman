...
@section('content')
	<div class="mb-8">
		<div class="flex items-center justify-between">
			<div>
				<h1 class="text-3xl font-bold text-gray-900 mb-2">Tambah Izin</h1>
				<p class="text-sm text-gray-600">Cipta kebenaran baharu untuk sistem</p>
			</div>
	...
	<div class="rounded-2xl border border-gray-200 bg-white shadow-lg">
		<div class="bg-gradient-to-r from-[#F0F7F0] to-[#F0F7F0]/80 border-b border-gray-200 px-6 py-4">
			<div class="flex items-center gap-3">
				...
				<div>
					<h2 class="text-lg font-bold text-gray-900">Maklumat Izin</h2>
					<p class="text-xs text-gray-600">Sila isi maklumat yang diperlukan</p>
				</div>
			</div>
		</div>
		<form action="{{ route('admin.permissions.store') }}" method="POST" class="p-6 max-w-md" id="permissionForm">
			@csrf
			...
			<div>
				...
				<input name="name" id="name" value="{{ old('name') }}" required class="mt-1 block w-full rounded-xl border-2 border-gray-200 px-4 py-3 shadow-sm focus:border-[#132A13] focus:ring-2 focus:ring-[#132A13]/20 sm:text-sm transition-all @error('name') border-red-300 focus:border-red-500 focus:ring-red-500/20 @enderror" placeholder="Contoh: view complaints" />
				@error('name')
