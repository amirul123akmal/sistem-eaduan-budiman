...
@section('content')
	<div class="mb-8">
		<div class="flex items-center justify-between">
			<div>
				<h1 class="text-3xl font-bold text-gray-900 mb-2">Kemaskini Peranan</h1>
				<p class="text-sm text-gray-600">ID Peranan: #{{ $role->id }}</p>
			</div>
		...
		<div class="bg-gradient-to-r from-[#F0F7F0] to-[#F0F7F0]/80 border-b border-gray-200 px-6 py-4">
			<div class="flex items-center gap-3">
				...
				<div>
					...
					<p class="text-xs text-gray-600">Ubah maklumat peranan di bawah</p>
				</div>
			</div>
		</div>
		<form action="{{ route('admin.roles.update', $role) }}" method="POST" class="p-6" id="roleForm">
			@csrf
			@method('PUT')
			...
			<div class="grid gap-6 lg:grid-cols-3">
				<div class="lg:col-span-1">
					...
					<input name="name" id="name" value="{{ old('name', $role->name) }}" required class="mt-1 block w-full rounded-xl border-2 border-gray-200 px-4 py-3 shadow-sm focus:border-[#132A13] focus:ring-2 focus:ring-[#132A13]/20 sm:text-sm transition-all @error('name') border-red-300 focus:border-red-500 focus:ring-red-500/20 @enderror" />
					@error('name')
