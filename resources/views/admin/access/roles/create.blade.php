...
@section('content')
	<h1 class="mb-6 text-2xl font-semibold">Tambah Peranan</h1>

	<form action="{{ route('admin.roles.store') }}" method="POST" class="grid gap-6 lg:grid-cols-3">
		@csrf
			...
			<div class="grid grid-cols-1 gap-2 rounded border bg-white p-4 sm:grid-cols-2">
				@foreach ($permissions as $perm)
					<label class="flex items-center gap-2 text-sm">
						<input type="checkbox" name="permissions[]" value="{{ $perm }}" class="rounded border-gray-300 text-gray-900 focus:ring-gray-900">
						<span>{{ $perm }}</span>
					</label>
				@endforeach
			</div>
