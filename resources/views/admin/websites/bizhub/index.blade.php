...
@section('content')
	<div class="mb-8 flex items-center justify-between">
		...
		<a href="{{ route('admin.websites.bizhub.create') }}" class="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#132A13] to-[#2F4F2F] px-6 py-3 text-sm font-semibold text-white shadow-lg transition-all duration-300 hover:shadow-xl hover:scale-[1.02] transform">
	...
	<div class="mb-6 rounded-2xl border border-gray-200 bg-gradient-to-br from-[#F0F7F0] to-white p-6 shadow-lg">
		<form method="GET" action="{{ route('admin.websites.bizhub.index') }}" class="flex gap-4">
			<div class="flex-1">
				<div class="relative">
					...
					<input type="text" name="search" value="{{ request('search') }}" placeholder="Cari BizHub..." class="w-full pl-10 rounded-xl border-2 border-gray-200 py-2.5 text-sm shadow-sm focus:border-[#132A13] focus:ring-[#132A13] transition-all">
				</div>
			</div>
			...
			@if(request('search'))
				<a href="{{ route('admin.websites.bizhub.index') }}" class="rounded-xl border-2 border-gray-300 bg-white px-6 py-2.5 text-sm font-semibold text-gray-700 shadow-sm hover:bg-gray-50 transition-all">
					Reset
				</a>
			@endif
		</form>
	...
	<div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-lg">
				...
				<tbody class="divide-y divide-gray-200 bg-white">
					@foreach($items as $index => $item)
						<tr class="hover:bg-[#F0F7F0]/50 transition-colors">
							...
							<td class="px-6 py-4">
								<div class="flex items-center gap-3">
									<div class="w-10 h-10 rounded-lg bg-[#F0F7F0] flex items-center justify-center">
										<svg class="h-5 w-5 text-[#132A13]" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M10.394 2.08a1 1 0 00-.788 0l-7 3a1 1 0 000 1.84L5.25 8.051a.999.999 0 01.356-.257l4-1.714a1 1 0 11.788 1.838L7.667 9.088l1.94.831a1 1 0 00.787 0l7-3a1 1 0 000-1.838l-7-3z"></path></svg>
									</div>
									<div>
							...
							<td class="px-6 py-4">
								<div class="text-sm text-gray-600">{{ $item['contact'] ?? '-' }}</div>
							...
							<td class="whitespace-nowrap px-6 py-4 text-right">
								<div class="flex items-center justify-end gap-2">
									<a href="{{ route('admin.websites.bizhub.show', $item['id']) }}" class="inline-flex items-center gap-1.5 rounded-lg bg-[#132A13] px-3 py-1.5 text-xs font-semibold text-white shadow-sm hover:bg-[#2F4F2F] transition-all hover:scale-105 transform">
									...
									<a href="{{ route('admin.websites.bizhub.edit', $item['id']) }}" class="inline-flex items-center gap-1.5 rounded-lg border-2 border-[#F0F7F0] bg-[#F0F7F0] px-3 py-1.5 text-xs font-semibold text-[#132A13] shadow-sm hover:bg-[#F0F7F0] transition-all hover:scale-105 transform">
		...
		@else
			<div class="px-6 py-16 text-center">
				...
				<a href="{{ route('admin.websites.bizhub.create') }}" class="inline-flex items-center gap-2 rounded-2xl bg-gradient-to-br from-[#132A13] to-[#2F4F2F] px-6 py-3 text-sm font-semibold text-white shadow-lg transition-all duration-300 hover:shadow-xl hover:scale-[1.02] transform">
					<svg class="h-5 w-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z" clip-rule="evenodd"></path></svg>
					Tambah BizHub
				</a>
			</div>
		...
		<div class="mt-6 flex flex-col items-center justify-between gap-4 sm:flex-row">
			...
			<div class="flex items-center gap-2">
				@if(isset($pagination['current_page']) && $pagination['current_page'] > 1)
					<a href="{{ route('admin.websites.bizhub.index', ['page' => $pagination['current_page'] - 1, 'search' => request('search')]) }}" class="flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition-all hover:bg-[#F0F7F0] hover:border-[#132A13] hover:text-[#132A13]">
				...
				<div class="hidden sm:flex items-center gap-1">
					@for($i = 1; $i <= min($pagination['last_page'] ?? 1, 10); $i++)
						@if($i == ($pagination['current_page'] ?? 1))
							<span class="flex h-9 w-9 items-center justify-center rounded-lg bg-gradient-to-r from-[#132A13] to-[#2F4F2F] text-sm font-semibold text-white shadow-md">{{ $i }}</span>
						@else
							<a href="{{ route('admin.websites.bizhub.index', ['page' => $i, 'search' => request('search')]) }}" class="flex h-9 w-9 items-center justify-center rounded-lg border border-gray-300 bg-white text-sm font-medium text-gray-700 transition-all hover:bg-[#F0F7F0] hover:border-[#132A13] hover:text-[#132A13]">{{ $i }}</a>
						@endif
					@endfor
				</div>
