...
@section('content')
    @php
        ...
        $updateRoute = $isSuperAdmin ? 'admin.websites.aktiviti.update' : 'admin.panel.websites.aktiviti.update';
    @endphp
    ...
    <div class="mb-6">
        ...
        <div>
            ...
            <p class="text-sm sm:text-base text-gray-600">Kemaskini maklumat aktiviti dalam sistem</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
        ...
        <div class="p-6 sm:p-8">
            <form action="{{ route($updateRoute, ['aktiviti' => $item->activityID]) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PATCH')
                ...
                <div>
                    ...
                    <input type="text" name="nama_aktiviti" id="nama_aktiviti" placeholder="Masukkan nama aktiviti"
                        ...
                        required value="{{ old('nama_aktiviti', $item->title ?? '') }}">
                    @error('nama_aktiviti')
                ...
                <div>
                    ...
                    <textarea name="keterangan" id="keterangan" rows="4" placeholder="Masukkan keterangan aktiviti"
                        ...
                        required>{{ old('keterangan', $item->description ?? '') }}</textarea>
                    @error('keterangan')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="tarikh" id="tarikh"
                        ...
                        required value="{{ old('tarikh', $item->activity_date ? \Carbon\Carbon::parse($item->activity_date)->format('Y-m-d') : '') }}">
                    @error('tarikh')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
